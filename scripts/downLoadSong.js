...
const songFile ="cd ../music_server/music/" 
...
const addingShortUuid = (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    const oldPath = path.resolve(__dirname,'..', '..','music_server', 'music', fileName)
    const newPath = path.resolve(__dirname, '..', '..','music_server', 'music', newFileName)
  ...
  return new Promise(async (resolve, reject) => {
    ...
    exec(`${songFile} && yt-dlp ${song.link}`, (error, stdout, stderr) => {
      try {
        ...
        exec(`${songFile} && yt-dlp --get-filename ${song.link}`, (error, stdout, stderr) => {
          if (error) {
            console.log(`error: ${error.message}`);
            return;
          }
...
const idnTagging = (data) => {
  return new Promise((resolve, reject) => {
    ...
    let result = ""
    exec(`${songFile} && idntag "${data.tempFileName}"`, (error, stdout, stderr) => {
      try {
        if (error) {
          console.log(`error: ${error.message}`);
          return;
...
const readTags = async (data) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const fullPath = path.resolve(__dirname, '..','..','music_server','music', filename)
...
const soundcloudDownload = async (data) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(`${songFile} && yt-dlp --dump-json ${link}` , (error, stdout, stderr) => {
...
const youtubeDownload = async (link) => {
  exec(`${songFile} && yt-dlp --no-playlist -f bestaudio -x --audio-format mp3 ${link}`, { cwd: '../../music_server/music/' }, (error, stdout, stderr) => {
    if (error) {
      console.log(`error: ${error.message}`);
      return;
    }
